  ...
  class Client
    ...
    def connect(server_url : String = "ws://localhost:3000")
      begin
        ...
        @socket.not_nil!.on_message do |message|
          handle_server_message(message)
        end
        
        @socket.not_nil!.on_close do |code, reason|
          @connected = false
        ...
        if @state.uuid
          @socket.not_nil!.send(@state.uuid.not_nil!)
        else
          @socket.not_nil!.send("")
        end
    ...
    def run
      ...
      spawn do
        @socket.not_nil!.run if @socket && @connected
      end
